...
exports.handler = async () => {
  try {
    ...
    if (!Array.isArray(rows) || rows.length === 0) {
      return json(502, { success: false, error: 'Keine Marktdaten von aWATTar erhalten.' });
    }
    ...
    if (!Number.isFinite(marketprice) || !Number.isFinite(startsAtMs) || !Number.isFinite(endsAtMs)) {
      return json(502, { success: false, error: 'Marktdaten unvollstaendig.' });
    }
    ...
    return json(502, { success: false, error: 'Live-Marktdaten momentan nicht erreichbar.' });
  }
};
