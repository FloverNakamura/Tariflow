...
import { CalculationRequest, TarifModul14a, EVehicleConfig, LargeLoadConfig } from '../types/pvTypes';
...
function toModule(value: unknown): TarifModul14a {
  if (value === 'modul1' || value === 'modul2' || value === 'modul3') return value;
  return 'none';
}
...
function sanitizeVehicles(value: unknown, fallbackUseBidirectional: boolean): EVehicleConfig[] {
  ...
  return value
    ...
    .map((entry) => {
      ...
      return {
        ...
        annualKm: clamp(toNum(vehicle.annualKm, 12000), 100, 200_000),
...
function toBuildingType(value: unknown): 'EFH' | 'MFH' | 'Gewerbe' {
  if (value === 'EFH' || value === 'MFH' || value === 'Gewerbe') return value;
  return 'EFH';
}
...
export function validateAndSanitize(body: unknown): CalculationRequest {
  ...
  const hh = (b.household && typeof b.household === 'object')
    ? b.household as Record<string, unknown>
    : {};

  const plz = String(hh.plz ?? '');
  if (!PLZ_RE.test(plz)) {
    throw new Error(
      `Ungültige PLZ "${plz}". Bitte eine gültige 5-stellige deutsche Postleitzahl angeben.`
    );
  }
  ...
  const annualConsRaw = hh.annualConsumption_kwh != null
    ...
    : NaN;
  const annualConsumption_kwh: number | undefined = Number.isFinite(annualConsRaw) && annualConsRaw > 0
    ? clamp(annualConsRaw, 500, 100_000)   // plausible range
    : undefined; // will fall back to BDEW profile

  // ── PV ─────────────────────────────────────────────────────────────────────
  const pvRaw = (b.pv && typeof b.pv === 'object') ? b.pv as Record<string, unknown> : {};
  ...
  // ── Storage ────────────────────────────────────────────────────────────────
  const stRaw = (b.storage && typeof b.storage === 'object') ? b.storage as Record<string, unknown> : {};
  ...
  // ── Heat Pump ──────────────────────────────────────────────────────────────
  const hpRaw = (b.heatPump && typeof b.heatPump === 'object') ? b.heatPump as Record<string, unknown> : {};
  const hasHeatPump = toBool(hpRaw.hasHeatPump, false);

  const heatPump = {
    hasHeatPump,
    annualConsumption_kwh: hasHeatPump
      ? clamp(toNum(hpRaw.annualConsumption_kwh, 3000), 100, 50_000)
      : 0,
  ...
  // ── E-Mobility ─────────────────────────────────────────────────────────────
  const evRaw = (b.emobility && typeof b.emobility === 'object') ? b.emobility as Record<string, unknown> : {};
  ...
  const legacyAnnualKm = hasEV ? clamp(toNum(evRaw.annualKm, 12000), 100, 200_000) : 0;
  ...
  // ── Tariff ─────────────────────────────────────────────────────────────────
  const taRaw = (b.tariff && typeof b.tariff === 'object') ? b.tariff as Record<string, unknown> : {};
