...
import { fetchPvDataWithSource } from './pvService';
import { createDynamicTarifEligibilityReport } from './dynamicTarifEligibility';

const HOURS_PER_YEAR = 365 * 24;
const YEAR = 2025;

type BuildingType = 'EFH' | 'MFH' | 'Gewerbe';

interface LoadProfilesData {
	annualBaseByPersons_kwh: Record<string, number>;
	buildingTypeMultiplier: Record<BuildingType, number>;
	monthFactors: number[];
	weekdayHourFactors: number[];
	weekendHourFactors: number[];
	heatPump: {
		monthFactors: number[];
		hourFactors: number[];
	};
	ev: {
		nightHourFactors: number[];
		eveningHourFactors: number[];
	};
}

interface SpotPriceData {
	monthlyBaseCt: number[];
	hourlyAddCt: number[];
	weekdayPremiumCt: number;
	weekendDiscountCt: number;
	monthlyVolatilityCt: number[];
}

interface TariffData {
	feedIn: {
		default_ct_per_kwh: number;
		smallSystemUpTo10kWp_ct_per_kwh?: number;
		largeSystemOver10kWp_ct_per_kwh?: number;
	};
	staticTariffs: Array<{
		name: string;
		label: string;
		energy_ct_per_kwh: number;
		network_ct_per_kwh: number;
		base_eur_per_year: number;
		meter_eur_per_year: number;
	}>;
	twoRateTariffs: Array<{
		name: string;
		label: string;
		ht_hours: number[];
		energy_ht_ct_per_kwh: number;
		energy_nt_ct_per_kwh: number;
		network_ht_ct_per_kwh: number;
		network_nt_ct_per_kwh: number;
		base_eur_per_year: number;
		meter_eur_per_year: number;
	}>;
	dynamicTariff: {
		name: string;
		label: string;
		spotMarkup_ct_per_kwh: number;
		taxes_and_levies_ct_per_kwh: number;
		network_ct_per_kwh: number;
		base_eur_per_year: number;
		meter_eur_per_year: number;
	...
	modules14a: Record<string, {
		label: string;
		type: 'none' | 'annual_discount' | 'network_time_window_discount' | 'hybrid';
		annual_discount_eur?: number;
		discount_ct_per_kwh?: number;
		windows?: Array<{ startHour: number; endHour: number }>;
	}>;
}

let loadProfilesCache: LoadProfilesData | null = null;
let spotPriceCache: SpotPriceData | null = null;
let tariffDataCache: TariffData | null = null;
...
function getLoadProfiles(): LoadProfilesData {
	if (!loadProfilesCache) {
		loadProfilesCache = readJson<LoadProfilesData>('../data/loadProfiles.json');
	}
	return loadProfilesCache;
}

function getSpotData(): SpotPriceData {
	if (!spotPriceCache) {
		spotPriceCache = readJson<SpotPriceData>('../data/spotPrices2025.json');
	}
	return spotPriceCache;
}

function getTariffData(): TariffData {
	if (!tariffDataCache) {
		tariffDataCache = readJson<TariffData>('../data/tariffData.json');
	}
...
function createHoursTemplate(): number[] {
...
function monthIndexByHour(hourIdx: number): number {
	const date = new Date(Date.UTC(YEAR, 0, 1, 0, 0, 0));
	date.setUTCHours(date.getUTCHours() + hourIdx);
	return date.getUTCMonth();
}

function buildBaseHouseholdAnnualConsumption(persons: number, buildingType: BuildingType): number {
...
function buildGeneralLoadProfile(annualKwh: number): number[] {
	const profiles = getLoadProfiles();
...
function buildHeatPumpProfile(annualKwh: number, cop?: number): number[] {
	const profiles = getLoadProfiles();
...
function buildLargeLoadProfile(annualKwh: number, curveKw: number[]): number[] {
...
function buildSpotSeries(): number[] {
...
interface StorageSimulationResult {
	gridDraw: number[];
	gridFeed: number[];
...
function simulateStorageAndFlows(
	load: number[],
	pv: number[],
	storage: CalculationRequest['storage'],
	spotPrices: number[],
	...
	for (let h = 0; h < HOURS_PER_YEAR; h++) {
		...
		const chargeFromPv = Math.min(
			pvSurplus,
			maxPower,
			Math.max(0, (capacity - soc) / etaCharge),
		);
		...
		if (dynamic && isCheap) {
			const extraGridCharge = Math.min(
				maxPower - chargeFromPv,
				Math.max(0, (capacity * 0.92 - soc) / etaCharge),
			);
...
function applyBidirectionalShift(
	gridDrawInput: number[],
	spotPrices: number[],
	enabled: boolean,
	annualEvConsumptionKwh: number,
): { gridDraw: number[]; shiftedKwh: number } {
...
function calculateModuleDiscountEur(
	moduleKey: TarifModul14a,
	networkCtPerKwh: number[],
	gridDraw: number[],
): number {
	const tariffData = getTariffData();
	const module = tariffData.modules14a[moduleKey];
	if (!module || module.type === 'none') return 0;
...
function evaluateTariffs(
	...
	autarkyRatePct: number,
	spotPricesCtPerKwh: number[],
): TariffResult[] {
	const tariffData = getTariffData();
...
function buildSachsenTariffComparison(
	request: CalculationRequest,
	annualConsumptionKwh: number,
	inferredSteerableConsumptionKwh: number,
): SachsenTariffComparisonResult {
	...
	const rows: SachsenTariffComparisonResult['rows'] = [];
...
export async function runCalculation(request: CalculationRequest): Promise<CalculationResponse> {
	const household = request.household;
	const coords = getCoordsFromPlz(household.plz);

	const manualAnnualGiven = Number.isFinite(household.annualConsumption_kwh) && (household.annualConsumption_kwh || 0) > 0;
	const householdEstimatedAnnual = buildBaseHouseholdAnnualConsumption(
		household.persons,
	...
	let pvSource: 'pvgis-live' | 'fallback-static' = 'fallback-static';
	...
	if (request.pv.hasPv && (request.pv.peakpower_kwp || 0) > 0) {
		...
		const pvSeries = await fetchPvDataWithSource(pvRequest);
	...
	const bidi = applyBidirectionalShift(
		storageResult.gridDraw,
		spotPrices,
		Boolean(request.emobility.useBidirectional),
		evAnnual,
	);
	...
	const tariffs = evaluateTariffs(
		...
		rates.autarky,
		spotPrices,
	).sort((a, b) => a.netCost_eur - b.netCost_eur);
	...
	const eligibilityReport = createDynamicTarifEligibilityReport({
