...
interface EligibilityInput {
	...
	staticNetCostEur: number;
	dynamicNetCostEur: number;
	dynamicSpreadCtPerKwh: number;
}
...
export function createDynamicTarifEligibilityReport(input: EligibilityInput): DynamicTarifEligibilityReport {
	...
	checks.push(check(
		'req_consumption_floor',
		'Ausreichender Jahresverbrauch',
		'requirements',
		annualConsumption >= 2500,
		'required',
		annualConsumption >= 2500
			? 'Verbrauch liegt in einem Bereich, in dem Preiszeitfenster relevant sind.'
			: 'Bei sehr niedrigem Verbrauch ist der dynamische Mehrwert oft zu klein.',
		round(annualConsumption),
	));

	checks.push(check(
		'tech_smart_meter_assumption',
		'Smart-Meter / Lastverschiebung technisch moeglich',
		'technical',
		input.dynamicOptimization || input.hasStorage || input.hasEv,
		'required',
		input.dynamicOptimization || input.hasStorage || input.hasEv
			? 'Technische Flexibilitaet fuer zeitvariable Preise ist vorhanden.'
			: 'Ohne Flexibilitaet wird ein dynamischer Tarif oft nicht ausgenutzt.',
		input.dynamicOptimization || input.hasStorage || input.hasEv,
	));

	checks.push(check(
		'tech_price_spread',
		'Ausreichende Marktpreis-Spreizung',
		'technical',
		input.dynamicSpreadCtPerKwh >= 6,
		'required',
		input.dynamicSpreadCtPerKwh >= 6
			? 'Die Spotpreisspreizung ist gross genug fuer Optimierung.'
			: 'Geringe Spotpreisspreizung reduziert das Einsparpotenzial.',
		round(input.dynamicSpreadCtPerKwh),
	));
	...
	checks.push(check(
		...
		'Speicher als Puffer',
		'technical',
		!input.hasStorage || input.storageCapacityKwh >= 4,
		'recommended',
		input.hasStorage
			? 'Speicherkapazitaet unterstuetzt Lastverschiebung.'
			: 'Auch ohne Speicher moeglich, aber mit weniger Optimierungsspielraum.',
		round(input.storageCapacityKwh),
	));

	checks.push(check(
		'exc_low_grid_draw',
		'Nicht zu niedriger Netzbezug',
		'exclusions',
		gridDraw >= 800,
		'required',
		gridDraw >= 800
			? 'Es bleibt genug netzseitiger Energieanteil fuer Tarifoptimierung.'
			: 'Sehr niedriger Netzbezug begrenzt den Effekt von Tarifwechseln.',
		round(gridDraw),
	));
	...
	checks.push(check(
		...
		estimatedSavings > 0
			...
			: 'Kein Kostenvorteil gegenueber statischem Tarif.',
