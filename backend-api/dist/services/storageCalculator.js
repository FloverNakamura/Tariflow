...
exports.PvCalculator = exports.StorageCalculator = void 0;
class StorageCalculator {
    constructor() {
        // ========== KONSTANTEN ==========
        // Preise (ct/kWh brutto)
        this.FIX_PRICE_EUR_KWAH = 0.27; // Sachsen Energy Fixpreis
        this.SPOT_PRICE_AVG_EUR_KWAH = 0.12; // Durchschnittlicher Spotpreis 2026
        // Betriebsparameter
        this.DAILY_CYCLES = 1.5; // Lade-/Entladzyklen pro Tag
        this.EFFICIENCY_ROUNDTRIP = 0.90; // Rundengang-Wirkungsgrad (90%)
        // Steuerliche Parameter
        this.IAB_TAX_RATE = 0.35; // Körperschaftsteuer + Gewerbesteuer
        this.IAB_DEDUCTION_BASE = 0.50; // 50% AfA-Satz
        this.IAB_DEPRECIATION_YEARS = 10; // Nutzungsdauer in Jahren
        // Netzentgelt-Schwellenwert (§14a StromNEV)
        this.RLM_THRESHOLD_MONTHLY = 8300; // kWh pro Monat
        this.PEAK_SHAVING_REDUCTION_RATE = 0.15; // 15% Netzentgelt-Einsparung
        // Verfügbare Speichergröße in kWh
        this.AVAILABLE_SIZES = [10, 15, 20, 25, 30, 35, 40, 45, 50, 54];
    }
    /**
    ...
    calculate(input) {
        ...
        const peakShavingPotential = this.checkPeakShavingPotential(input.monthlyConsumption_kwh || monthlyAvgConsumption, storageSizing.recommended_kwh, input.investmentCost_eur_per_kwh);
        return {
     ...
     */
    calculateDailyConsumption(annualConsumption) {
     ...
     */
    dimensionStorage(dailyConsumption) {
     ...
     */
    calculateArbitrageRevenue(storageSize_kwh) {
     ...
     */
    calculateTaxBenefit(investmentCost_eur) {
     ...
     */
    calculatePaybackPeriod(investment, annualBenefit) {
        if (annualBenefit === 0)
            return Infinity;
     ...
     */
    checkPeakShavingPotential(monthlyConsumption, storageSize_kwh, investmentCost_eur_per_kwh) {
...
exports.StorageCalculator = StorageCalculator;
/**
...
class PvCalculator {
    constructor() {
        ...
        this.PV_COST_TIERS = [
            { maxKwp: 30, cost_eur_kwp: 1150 }, // <30 kWp: 1.150 €/kWp
            { maxKwp: 100, cost_eur_kwp: 950 }, // 30–100 kWp: 950 €/kWp
            { maxKwp: Infinity, cost_eur_kwp: 750 }, // >100 kWp: 750 €/kWp
        ];
        // ── Betriebsparameter ──
        this.SPECIFIC_YIELD_KWH_KWP = 950; // kWh/kWp/Jahr (PVGIS, DE-Ø)
        this.DEGRADATION_RATE = 0.005; // 0,5 %/Jahr (IEC Norm)
        this.OM_COST_EUR_KWP_YEAR = 15; // €/kWp/Jahr (ca. 1,5 % Investkosten)
        this.PV_LIFETIME_YEARS = 25; // Systemlebensdauer
        this.ROOFTOP_M2_PER_KWP = 7; // m² Dachfläche pro kWp (Faustformel)
        // ── Eigenverbrauchsquoten (HTW Berlin / Quaschning 2023) ──
        this.SELF_CONSUMPTION_WITHOUT_STORAGE = 0.30; // 30% direkte Eigenverbrauchsquote
        this.SELF_CONSUMPTION_WITH_STORAGE = 0.70; // 70% mit optimiertem Speicher
        // ── Preise (geerbt aus StorageCalculator-Kontext) ──
        this.FIX_PRICE_EUR_KWH = 0.27; // Sachsen Energy Fixpreis
        this.SPOT_PRICE_AVG_EUR_KWH = 0.12; // Spotpreis-Ø 2026
        this.STORAGE_DAILY_CYCLES = 1.5;
        this.STORAGE_EFFICIENCY = 0.90;
        // ── Steuer ──
        this.IAB_DEDUCTION_BASE = 0.50; // 50% AfA-Basis
        this.IAB_TAX_RATE = 0.35; // KöSt + GewSt
    }
    /**
     * Berechne PV-Wirtschaftlichkeit für beide Szenarien (ohne/mit Speicher)
     * und gib eine Empfehlung aus.
     */
    calculateCombined(input) {
    ...
    calculate(input) {
        ...
        if (withStorage) {
            ...
            storageBonus = {
                ...
                selfConsumptionIncrease_percent: parseFloat(((this.SELF_CONSUMPTION_WITH_STORAGE - this.SELF_CONSUMPTION_WITHOUT_STORAGE) * 100).toFixed(1)),
     ...
     * Gibt ct/kWh zurück
     */
    getFeedInTariff(pvKwp) {
        for (const tier of this.EEG_FEED_IN_TIERS) {
            if (pvKwp <= tier.maxKwp)
                return tier.rate_ct_kwh;
        }
     ...
     * Gibt €/kWp zurück
     */
    getInstallCost(pvKwp) {
        for (const tier of this.PV_COST_TIERS) {
            if (pvKwp <= tier.maxKwp)
                return tier.cost_eur_kwp;
        }
...
exports.PvCalculator = PvCalculator;
