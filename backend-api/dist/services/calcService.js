...
function getAspectFactor(aspect) {
    const table = pvProfile.aspectCorrectionFactors;
    const keys = Object.keys(table).map(Number).sort((a, b) => a - b);
    if (aspect <= keys[0])
        return table[String(keys[0])];
    if (aspect >= keys[keys.length - 1])
...
function getAngleFactor(angle) {
    const table = pvProfile.angleCorrectionFactors;
    const keys = Object.keys(table).map(Number).sort((a, b) => a - b);
    if (angle <= keys[0])
        return table[String(keys[0])];
    if (angle >= keys[keys.length - 1])
        return table[String(keys[keys.length - 1])];
    for (let i = 0; i < keys.length - 1; i++) {
...
function generatePvHourly(peakpower_kwp, angle, aspect, loss_pct) {
    const specificYield = pvProfile.specificYield_kwh_per_kwp;
    ...
    const hourly = [];
    for (let m = 0; m < 12; m++) {
        const monthFactor = pvProfile.monthlyYieldFactors[m];
        const monthKey = MONTHS[m];
        const monthYield_kwh = (annualYield_kwh / 12) * monthFactor;
        const dayFactors = pvProfile.hourlyYieldFactors[monthKey];
        const daySum = dayFactors.reduce((a, b) => a + b, 0);
...
function getAnnualHouseholdConsumptionForPersons(persons) {
    const lookup = loadProfiles.annualConsumption_kwh;
    ...
    if (persons <= 1)
        return 1500;
    if (persons <= 4)
        return 1500 + ((persons - 1) * 1000);
    return 5500 + ((persons - 5) * 800);
}
function getEVVehicles(req) {
...
// ── Speicher-Simulation ──────────────────────────────────────────────────────
function simulateStorage(pvHourly, consHourly, capacity_kwh, maxPower_kw, efficiency, spotHourly, useDynamic, spotAnnualAvgCt) {
...
function buildSpotArrayStatic() {
    const arr = [];
    for (let m = 0; m < 12; m++) {
        const avgPrice = spotPrices.monthlyAvg_ct_per_kwh[m];
        const factors = spotPrices.hourlyProfileFactors[MONTHS[m]];
        const days = DAYS_PER_MONTH[m];
        for (let d = 0; d < days; d++) {
            for (let h = 0; h < 24; h++) {
...
function buildSpotArrayFromHourlyAverages(hourlyAvgCt) {
    const arr = [];
    for (let m = 0; m < 12; m++) {
        const days = DAYS_PER_MONTH[m];
        for (let d = 0; d < days; d++) {
...
async function buildSpotArrayLiveOrFallback() {
    try {
        ...
        for (const row of rows) {
            ...
            if (!Number.isFinite(ts) || !Number.isFinite(marketPriceEurPerMwh))
                continue;
        ...
        const fallbackHourAvg = knownValues.length ? avg(knownValues) : spotPrices.annualAvg_ct_per_kwh;
        const hourlyAvgCt = Array.from({ length: 24 }, (_, hour) => {
            return latestByHour.has(hour) ? latestByHour.get(hour) : fallbackHourAvg;
        });
...
function getNetworkCost_ct(hour, module14a) {
...
function getNetworkTariffOnly_ct(hour, module14a) {
    const base = tariffData.networkCosts;
    const h = hour % 24;
    if (module14a === 'modul1')
        return base.networkTariff_ct_per_kwh; // kein Per-Unit-Rabatt bei Modul 1
    if (module14a === 'modul2') {
        const peak = tariffData.module14a.modul2.peakHours.includes(h);
...
function getModuleLabel(module14a) {
    if (module14a === 'none')
        return 'Kein §14a-Modul';
    return tariffData.module14a[module14a]?.label || 'Kein §14a-Modul';
}
function getTariffLabel(tariffType) {
    if (tariffType === 'dynamic')
        return tariffData.dynamicTariff.label;
    if (tariffType === 'twoRate')
        return tariffData.twoRateTariff.label;
    return tariffData.staticTariff.label;
}
function getCombinedTariffLabel(tariffType, module14a) {
...
function calcTariffCost(gridDraw, gridFeed, selfCons, spotArr, label, name, tariffType, module14a, peakpower_kwp, totalConsumption_kwh) {
    ...
    for (let i = 0; i < 8760; i++) {
        ...
        if (isDynamic) {
            ...
            const ntHours = tariffData.twoRateTariff.ntHours || [22, 23, 0, 1, 2, 3, 4, 5];
...
function buildMonthly(pvH, consH, drawH, feedH, selfH) {
    const monthly = [];
    let idx = 0;
    for (let m = 0; m < 12; m++) {
...
function applyBiDiShift(gridDraw, spotArr, annualShift_kwh) {
...
// ── Hauptfunktion ─────────────────────────────────────────────────────────────
async function runCalculation(req) {
    const { lat, lon } = (0, geocodeService_1.getCoordsFromPlz)(req.household.plz);
    const buildingType = req.household.buildingType ?? 'EFH';
    ...
    if (peakpower > 0) {
        try {
            const pvData = await (0, pvService_1.fetchPvData)({ lat, lon, peakpower, angle, aspect, loss });
    ...
    // Speichersimulation
    let gridDraw, gridFeed, selfCons;
    ...
    for (const tariffType of candidateTariffs) {
        for (const module14a of candidateModules) {
            tariffs.push(calcTariffCost(gridDraw, gridFeed, selfCons, spotArr, getCombinedTariffLabel(tariffType, module14a), `${tariffType}-${module14a}`, tariffType, module14a, peakpower, totalCons_kwh));
        }
    }
